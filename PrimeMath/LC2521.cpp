...
int distinctPrimeFactors(vector<int> &nums)
{
    ...
    int n = nums.size();
    for (int i = 0; i < n; i++)
        maxi = max(maxi, nums[i]);  // finding max
    ...
    for (int i = 2; i < prime.size(); i++)
        if (prime[i] == 1)
            primeno.push_back(i);         // extracting only prime no like 2 ,3 ,5, 7
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < primeno.size(); j++)
        {
            if (nums[i] < primeno[j])
                break;
            if (nums[i] % primeno[j] == 0)
                freq[j] = 1; // filling existence of an prime no
        }
    }
